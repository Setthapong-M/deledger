...
try {
  ...
  else if (run("pnpm", ["db:migrate"], {DATABASE_URL: testEnv.TEST_ADMIN_DATABASE_URL}) !== 0) process.exitCode = 1;
  else {
    status = run("pnpm", ["--dir", "api", "exec", "vitest", "run", "--coverage", "--reporter=dot"]);
    if (status === 0) status = run("pnpm", ["--dir", "web", "exec", "vitest", "run", "--coverage", "--reporter=dot"]);
  }
} finally {
