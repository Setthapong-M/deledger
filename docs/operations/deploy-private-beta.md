...
2. Create four file-backed Compose secrets for the PostgreSQL admin and three Deledger roles under `/etc/deledger/secrets` (or the `DELEDGER_SECRET_DIR` recorded in the runtime environment), with no world permissions. Keep values outside this repository. Build the database image as `deledger-postgres:latest` (or set the reviewed equivalent as `DELEDGER_DB_IMAGE` in `/etc/deledger/backup.env` for restore verification) and build the isolated `backup` image.
