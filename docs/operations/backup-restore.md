...
Weekly verification decrypts the newest checksum-valid artifact into a uniquely named, network-isolated PostgreSQL container using the same PostgreSQL 18.6 version as the application database. It checks the Prisma migration head, nine-table boundary (including phone identities and local sessions), forced RLS, constraints, and executes a row-count query for every expected table before removing only its own generated container and volume. A failed or overdue verification makes readiness fail closed.
