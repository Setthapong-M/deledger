...
import type { MonthKey, MonthView } from "../domain/contracts.js";
import { DomainError } from "../domain/errors.js";
import { getMonthView, listMonthKeys } from "../repositories/months.js";
...
export async function readMonth(client: Prisma.TransactionClient, ownerId: string, monthStart: string): Promise<MonthView> {
  const view = await getMonthView(client, ownerId, monthStart);
  if (!view) throw new DomainError("MONTH_NOT_FOUND", "ไม่พบเดือนนี้");
  return view;
}

export async function listHistory(client: Prisma.TransactionClient, ownerId: string, options: { before?: string; limit?: number } = {}): Promise<HistoryEntry[]> {
  ...
  const archiveRows = await client.user_archive_period.findMany({where:{owner_id:ownerId},orderBy:{archived_at:'desc'}});
  const gaps: TrackingGap[] = archiveRows
    .filter(row=>row.restored_at!==null && monthOf(row.archived_at.toISOString())<monthOf(row.restored_at.toISOString()))
    .map(row=>({kind:'tracking_gap',id:`gap:${row.id}`,archivedAt:row.archived_at.toISOString(),restoredAt:row.restored_at?.toISOString() ?? null}));
