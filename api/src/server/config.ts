...
export function loadConfig(environment: ConfigEnvironment = process.env): AppConfig {
  const result = schema.safeParse({
    ...
    IDENTITY_DATABASE_URL: environment.IDENTITY_DATABASE_URL,
  ...
  const database = new URL(result.data.DATABASE_URL);
  const identityDatabase = new URL(result.data.IDENTITY_DATABASE_URL);
  if (database.username !== "deledger_web" || identityDatabase.username !== "deledger_identity" || identityDatabase.protocol !== "postgresql:" || identityDatabase.hostname !== database.hostname || identityDatabase.port !== database.port || identityDatabase.pathname !== database.pathname) {
    throw new Error("database credentials must use deledger_web and deledger_identity on the same database");
  }
