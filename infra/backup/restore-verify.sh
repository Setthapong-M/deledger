...
restore_image="${DELEDGER_DB_IMAGE:-postgres:18.6-bookworm}"
docker image inspect "$restore_image" >/dev/null 2>&1 || { echo "restore database image is missing" >&2; exit 2; }

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
expected_migration="$(find "$repo_root/api/prisma/migrations" -mindepth 1 -maxdepth 1 -type d -printf '%f\n' | sort | tail -n 1)"
...
docker run --name "$label" --detach --network none --volume "${label}-data:/var/lib/postgresql" \
  ...
  "$restore_image" postgres >/dev/null
...
[[ "$(run_sql "SELECT migration_name FROM public._prisma_migrations WHERE finished_at IS NOT NULL AND rolled_back_at IS NULL ORDER BY started_at DESC LIMIT 1")" == "$expected_migration" ]] || { echo "restore migration head mismatch" >&2; exit 1; }
[[ "$(run_sql "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('app_user','user_identity_email','user_identity_phone','local_session','user_archive_period','reporting_month','balance_snapshot','monthly_recurring_expense','monthly_expense_detail')")" == "9" ]] || { echo "restore table boundary mismatch" >&2; exit 1; }
[[ "$(run_sql "SELECT count(*) FROM pg_class AS c JOIN pg_namespace AS n ON n.oid = c.relnamespace WHERE n.nspname = 'public' AND c.relkind = 'r' AND c.relname IN ('app_user','user_identity_email','user_identity_phone','local_session','user_archive_period','reporting_month','balance_snapshot','monthly_recurring_expense','monthly_expense_detail') AND c.relrowsecurity AND c.relforcerowsecurity")" == "9" ]] || { echo "restore RLS boundary mismatch" >&2; exit 1; }
[[ "$(run_sql "SELECT count(*) FROM information_schema.table_constraints WHERE table_schema = 'public' AND table_name IN ('app_user','user_identity_email','user_identity_phone','local_session','user_archive_period','reporting_month','balance_snapshot','monthly_recurring_expense','monthly_expense_detail') AND constraint_type IN ('PRIMARY KEY','FOREIGN KEY','CHECK','UNIQUE')")" -gt 0 ]] || { echo "restore constraints are missing" >&2; exit 1; }
