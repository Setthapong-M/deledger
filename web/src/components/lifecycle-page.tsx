...
export function LifecyclePage({ mode }: { mode: "start" | "resume" }) {
  ...
  if (state === "checking") return <AppShell><section className="card empty-state" aria-live="polite"><p>กำลังตรวจสอบสิทธิ์…</p></section></AppShell>;
  if (state === "error") return <AppShell><section className="card empty-state"><h1>เข้าถึงหน้านี้ไม่ได้</h1><p>{error}</p><button className="primary-button" type="button" onClick={() => window.location.reload()}>ลองใหม่</button></section></AppShell>;
  return <AppShell><section className="lifecycle-page card" aria-labelledby={`${mode}-title`}><p className="eyebrow">{mode === "start" ? "เริ่มต้นติดตาม" : "กลับมาเริ่มใหม่"}</p><h1 id={`${mode}-title`}>{mode === "start" ? "เริ่มบัญชีรายรับรายจ่าย" : "เริ่มติดตามจากเดือนนี้"}</h1><p>{mode === "start" ? "กรอกยอดที่รู้ตอนนี้ แล้วค่อยเติมรายละเอียดระหว่างเดือนได้ ไม่ต้องรอรอบหรือจำทุกรายการ" : "ช่วงก่อนหน้าจะถูกทำเครื่องหมายเป็นช่วงข้อมูลขาด ไม่ต้องย้อนสร้างรายการเดิม"}</p><LifecycleForm mode={mode} /></section></AppShell>;
}
