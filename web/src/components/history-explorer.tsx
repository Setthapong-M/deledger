...
import { api, ApiClientError, type HistoryEntry } from "@/lib/api-client";
import { formatDate, formatMoney, formatMonth } from "@/lib/format";
import { StatusBadge } from "./status-badge";
import { FeedbackBanner } from "./feedback-banner";

export function HistoryExplorer({ initialEntries = [] }: { initialEntries?: HistoryEntry[] }) {
  ...
  useEffect(() => {
    ...
    void api.history().then((next) => { setEntries(next); setSelectedId(next[0]?.id ?? ""); }).catch((reason) => setMessage(reason instanceof Error ? reason.message : "โหลดประวัติไม่สำเร็จ")).finally(() => setLoading(false));
  ...
  return <section className="history-explorer" aria-label="ประวัติรายเดือน" onKeyDown={keyNavigate}>
    ...
    <div className="cover-flow" tabIndex={0}>
      ...
      <Cover entry={selected} onRefresh={(next) => setEntries((current) => current.map((entry) => entry.id === next.id ? next : entry))} setMessage={setMessage} />
...
function Cover({ entry, onRefresh, setMessage }: { entry: HistoryEntry; onRefresh: (entry: HistoryEntry) => void; setMessage: (message: string | null) => void }) {
  ...
  async function refresh() {
    try { onRefresh({ kind: "month", id: view.month, view: await api.month(view.month) }); setMessage(null); } catch (reason) { if (reason instanceof ApiClientError && reason.current) onRefresh({ kind: "month", id: view.month, view: reason.current }); setMessage(reason instanceof Error ? reason.message : "โหลดข้อมูลล่าสุดไม่สำเร็จ"); }
  }
