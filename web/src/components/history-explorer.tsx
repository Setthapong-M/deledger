...
export function HistoryExplorer({ initialEntries = [], onSessionExpired }: { initialEntries?: HistoryEntry[]; onSessionExpired?: () => void }) {
  ...
  return <section className="history-explorer" aria-label="ประวัติรายเดือน" onKeyDown={keyNavigate}>
    ...
    <div className="filmstrip" role="tablist" aria-label="เลือกเดือน"><button type="button" className="filmstrip-arrow" aria-label="เดือนก่อนหน้า" onClick={() => selectAt(index - 1)} disabled={index <= 0}>‹</button>{entries.map((entry, entryIndex) => <button key={entry.id} type="button" role="tab" aria-selected={entry.id === selected.id} aria-current={entry.id === selected.id ? "true" : undefined} className={`film-card${entry.id === selected.id ? " is-selected" : ""}${entry.kind === "tracking_gap" ? " is-gap" : ""}`} onClick={() => selectAt(entryIndex)}><span>{entry.kind === "month" ? formatMonth(entry.id) : "ช่วงข้อมูลขาด"}</span><small>{entry.kind === "month" ? statusText[entry.view.reconciliation.state] : "หยุดติดตาม"}</small></button>)}<button type="button" className="filmstrip-arrow" aria-label="เดือนถัดไป" onClick={() => selectAt(index + 1)} disabled={index >= entries.length - 1}>›</button></div>
    ...
    <p className="helper-text history-hint">เลือกเดือนจากแถบด้านบน หรือใช้ปุ่มลูกศรเพื่อดูเดือนอื่น</p>
  </section>;
}
...
function Cover({ entry, onRefresh, setMessage, onSessionExpired }: { entry: HistoryEntry; onRefresh: (entry: HistoryEntry) => void; setMessage: (message: string | null) => void; onSessionExpired: () => void }) {
  if (entry.kind === "tracking_gap") return <article className="cover-card tracking-gap-card"><span className="status-badge">ช่วงหยุดติดตาม</span><p className="eyebrow">ไม่มีการบันทึกรายเดือนในช่วงนี้</p><h1>ช่วงข้อมูลขาด</h1><dl className="detail-list"><div><dt>เริ่ม archive</dt><dd>{formatDate(entry.archivedAt.slice(0, 10))}</dd></div><div><dt>กู้คืน</dt><dd>{entry.restoredAt ? formatDate(entry.restoredAt.slice(0, 10)) : "ยังไม่กู้คืน"}</dd></div></dl><p className="helper-text">ไม่มีการคาดเดายอดเงินในช่วงนี้</p></article>;
