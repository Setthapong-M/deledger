...
export default function LoginPage() {
  ...
  return <AppShell><section className={`mx-auto mt-[8vh] mb-0 w-full max-w-[620px] [&>p:not(:first-child)]:leading-[1.65] [&>p:not(:first-child)]:text-muted-ink ${ui.card}`} aria-labelledby="login-title"><p className={ui.eyebrow}>DELEDGER LOCAL</p><h1 id="login-title">เข้าสู่ระบบ</h1>{error ? <p className={ui.fieldError} role="alert">{error}</p> : null}{mode?.environment === "local" ? <><p>เลือก User สำหรับการพัฒนา โดยใช้ email หรือเบอร์มือถือไทย</p><LocalLoginForm /></> : mode?.environment === "qas" ? <><p>QAS ใช้ Cloudflare Access ตามปกติ หลังผ่าน Cloudflare ให้เปิด Deledger อีกครั้ง</p><Link className={ui.primaryButton} href="/">กลับไป Deledger</Link></> : <p>กำลังตรวจสอบ environment…</p>}</section></AppShell>;
}
